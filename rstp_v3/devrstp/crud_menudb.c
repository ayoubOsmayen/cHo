...
void create_record(MYSQL *conn, const char *table_name, const char *ip_address, const char *session_id, const char *connection_start, const char *connection_end, const char *bandwidth_used) {
    char query[1024];
    snprintf(query, sizeof(query),
             "INSERT INTO %s (ip_address, session_id, connection_start, connection_end, bandwidth_used) "
             "VALUES ('%s', '%s', '%s', '%s', '%s')",
             table_name, ip_address, session_id, connection_start, connection_end, bandwidth_used);
...
void read_record(MYSQL *conn, const char *table_name, int id) {
    char query[256];
...
void update_record(MYSQL *conn, const char *table_name, int id, const char *ip_address, const char *session_id, const char *connection_start, const char *connection_end, const char *bandwidth_used) {
    char query[1024];
    snprintf(query, sizeof(query),
             "UPDATE %s SET ip_address = '%s', session_id = '%s', connection_start = '%s', "
             "connection_end = '%s', bandwidth_used = '%s' WHERE id = %d",
             table_name, ip_address, session_id, connection_start, connection_end, bandwidth_used, id);
...
void delete_record(MYSQL *conn, const char *table_name, int id) {
    char query[256];
...
void describe_table(MYSQL *conn, const char *table_name) {
    char query[256];
    snprintf(query, sizeof(query), "DESCRIBE %s", table_name);
    printf("\nDescription of table %s:\n", table_name);
    print_query_results(conn, query);
}
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        printf("7. Show tables\n");
        printf("8. Describe the table\n");
        printf("9. Exit\n");
        ...
        switch (choice) {
            case 1:
                ...
                scanf("%s", session_id);
                printf("Enter connection start (YYYY-MM-DD HH:MM:SS): ");
                scanf("%s", connection_start);
                printf("Enter connection end (YYYY-MM-DD HH:MM:SS): ");
                scanf("%s", connection_end);
                printf("Enter bandwidth used: ");
                scanf("%s", bandwidth_used);
                create_record(conn, table_name, ip_address, session_id, connection_start, connection_end, bandwidth_used);
                break;
            case 2:
                // Read a record by ID
                printf("Enter ID: ");
                scanf("%d", &id);
                read_record(conn, table_name, id);
                break;
            case 3:
                ...
                scanf("%s", session_id);
                printf("Enter new connection start (YYYY-MM-DD HH:MM:SS): ");
                scanf("%s", connection_start);
                printf("Enter new connection end (YYYY-MM-DD HH:MM:SS): ");
                scanf("%s", connection_end);
                printf("Enter new bandwidth used: ");
                scanf("%s", bandwidth_used);
                update_record(conn, table_name, id, ip_address, session_id, connection_start, connection_end, bandwidth_used);
                break;
            case 4:
                // Delete a record by ID
                printf("Enter ID: ");
                scanf("%d", &id);
                delete_record(conn, table_name, id);
                break;
            case 5:
                // Search by ID
                printf("Enter ID: ");
