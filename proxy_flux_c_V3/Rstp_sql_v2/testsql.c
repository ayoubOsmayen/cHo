...
void create_pipeline(const char *video_url, const char *display_text) {
    ...
    g_object_set(text_overlay, "valign", 2, NULL); // Centrage vertical
...
// Enregistrement des informations des clients RTSP dans MySQL
void log_client_info(const char *ip_address, const char *session_id, const char *bandwidth) {
    ...
    if (mysql_real_connect(conn, "localhost", "root", "password", "rtsp_db", 0, NULL, 0) == NULL) {
        fprintf(stderr, "mysql_real_connect() a échoué\n");
        mysql_close(conn);
        return;
    }

    char query[512];
    snprintf(query, sizeof(query), "INSERT INTO rtsp_clients (ip_address, session_id, connection_start, bandwidth_used) VALUES ('%s', '%s', NOW(), '%s')", ip_address, session_id, bandwidth);
    if (mysql_query(conn, query)) {
        fprintf(stderr, "INSERT INTO rtsp_clients a échoué. Erreur : %s\n", mysql_error(conn));
    }
